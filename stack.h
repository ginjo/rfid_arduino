...
#define __FUNCTION_STACK_H__
  ...
  public:
    ...
    virtual void push(CB func) {
      SK_LOG(5, F("Stack.push to index "), false); SK_LOG(5, stack_index+1, true);
      ...
      if (func) {
        ...
        LOG(3, F("Tried pushing nullptr to stack"), true);
      }
    }

    virtual CB pop() {
        SK_LOG(5, F("Stack.pop from index "), false); SK_LOG(5, stack_index, true);
    ...
    virtual CB top() {
        SK_LOG(5, F("Stack.top index "), false); SK_LOG(5, stack_index, true);
        if (stack_index < 0) return nullptr;
        return stack[stack_index];
    }

    virtual void call(void *dat = nullptr, bool _pop = false) {
        SK_LOG(5, F("Stack.call index "), false); SK_LOG(5, stack_index, true);
        CB cback = top();
    ...
    virtual void resetStack(CB func = nullptr) {
      SK_LOG(5, F("Stack.resetStack with index "), false); SK_LOG(5, stack_index, true);
      stack_index = -1;
